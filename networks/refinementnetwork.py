...
# def generate_distance_map(map_xor,points_center,points_bg,gt,patch_size_raw,patch_size_col):
...
# def iou_cal(pre, gts, patch_size, extreme_points, mask_thres=0.5):
def iou_cal(pre, gts, extreme_points, mask_thres=0.5):
    ...
    for jj in range(int(pre.shape[0])):
        ...
        iu_ave = iu_ave+iu
        # distance_map_new[jj, :, :, :] = generate_distance_map(map_xor, points_center, points_bg, gt, patch_size[0][jj], patch_size[1][jj])
...
class SegmentationNetwork(nn.Module):
    ...
    # def forward(self, input,IOG_points,gts,patch_size,refinement_num_max):
    def forward(self, input,IOG_points,gts,refinement_num_max):
        ...
        for refinement_num in range(0, refinement_num_max):
            distance_map = self.iog_points(distance_map)  # 就是蓝色的那部分卷积
            ...
            # iou_i, distance_map_new = iou_cal(out_512, gts, patch_size, distance_map_512)
