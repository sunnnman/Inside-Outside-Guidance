...
model_save_dir = './runs/IOG_pascal_'
log_dir = './runs/IOG_pascal_/log'
logger = setup_logger("semantic_segmentation", log_dir, filename='{}_log.txt'.format(modelName))
...
if resume_epoch == 0:  #这个是跑到一半如果中断什么的重新跑，net从mypath里加载的模型才是预训练模型
    ...
    net.load_state_dict(
        torch.load("/home/zhupengqi/Inside-Outside-Guidance-master/runs/pretrained_IOG_pascal_0513/models/IOG_pascal_epoch-49.pth",
                   map_location=lambda storage, loc: storage))
...
if resume_epoch != nEpochs:
    ...
    # voc_train = CocoSegmentation(split='train', transform=composed_transforms_tr)
    # voc_val = CocoSegmentation(split='val', transform=composed_transforms_ts)

    # voc_train = GrabCutDataset(dataset_path='/home/zhupengqi/datasets/GrabCut_train/', transform=composed_transforms_tr)
    # voc_val = GrabCutDataset(dataset_path='/home/zhupengqi/datasets/GrabCut_test/', transform=composed_transforms_ts)
