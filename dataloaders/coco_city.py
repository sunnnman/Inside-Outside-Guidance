...
class CocoSegmentation_City(data.Dataset):
    def __init__(self,
                 split,
                 area_range=[],
                 ...
                 db_root=Path.db_root_dir('coco_cityscapes'),
        ...
        self.root = os.path.join(db_root, 'val_images')
        # self.root = '/home/zhupengqi/dataset/cityscapes/datasets/cityscapes/leftImg8bit/train/'
        # annFile = os.path.join(db_root, 'annotations', 'instances_'+split+'.json')
        annFile = os.path.join(db_root, 'val.json')
        # annFile = os.path.join(db_root, 'train_.json')
        # 微调的city数据


        # self.root = os.path.join(db_root, 'leftImg8bit/train/')
        # annFile = os.path.join(db_root, 'train_.json')
        ...
        pascal_cat_name = ['car', 'pedestrian', 'truck', 'bus', 'rider']
    ...
    def __getitem__(self, index):
        ...
        if not self.return_rle:
            try:
                path_ = path
                if self.split == 'train':
                    path_ = path.split('_')[0] +'/' + path  # 这里的path是图片名字
                img = np.array(Image.open(os.path.join(self.root, path_)).convert('RGB')).astype(np.float32)
            ...
            _void_pixels = (target == 255).astype(np.float32)
            sample['void_pixels'] = _void_pixels
    ...
    def __str__(self):
        return 'CocoSegmentation_City(split='+str(self.split)+', area_range='+str(self.area_range)+\
