...
class VOCSegmentation(data.Dataset):
    ...
    def __init__(self,
        ...
        _mask_dir = os.path.join(_voc_root, 'SegmentationObject')#each object each color
    ...
    def _make_img_gt_point_pair(self, index):
        ...
        # Read Image
        _img = np.array(Image.open(self.images[_im_ii]).convert('RGB')).astype(np.float32) ###zsy open image imread 
