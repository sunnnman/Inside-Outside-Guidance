...
def distance_map(mask, pad=10, v=0.15, relax=False):
    ...
    if bbox is not None:  # 产生抖动 模拟用户交互
    ...
    guidemap = np.zeros((mask.shape[0], mask.shape[1], 2))
    dismap = np.zeros((mask.shape[0], mask.shape[1]))
    zeromap = np.zeros((mask.shape[0], mask.shape[1]))
...
def fixed_resize(sample, resolution, flagval=None):
    ...
    Returns:resize大小后的image

    '''
    if flagval is None:
        if ((sample == 0) | (sample == 1)).all():
            flagval = cv2.INTER_NEAREST  # 最近邻插值
        else:
            flagval = cv2.INTER_CUBIC  # 4x4像素邻域的双三次插值
