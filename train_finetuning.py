...
resume_epoch = 50 # Default is 0, change if want to resume
...
# modelName = 'Grid_coco_5010_guassian7_sample'
modelName = 'Grid_mscmr_5010_guassian7_sample_finetuning_50'
...
model_save_dir = './runs/Grid_mscmr_5010_guassian7_sample_finetuning'
log_dir = './runs/Grid_mscmr_5010_guassian7_sample_finetuning/log'
...
if resume_epoch == 0:  # 这个是跑到一半如果中断什么的重新跑，net从mypath里加载的模型才是预训练模型
    ...
    print('Initializing weights from:Grid_pascal_SBD_max70_gaussian7_hw_80%25_epoch-9.pth')
    net.load_state_dict(
        torch.load(
            "/home/zhupengqi/Inside-Outside-Guidance-master/runs/Grid_mscmr_5010_guassian7_sample_finetuning/models/Grid_mscmr_5010_guassian7_sample_finetuning_epoch-49.pth",
            map_location=lambda storage, loc: storage))
...
if resume_epoch != nEpochs:
    ...
    voc_train = mscmrDataset(dataset_path='/home/zhupengqi/dataset/mscmr/',transform=composed_transforms_ts_grids)
    voc_val = mscmrDataset(dataset_path='/home/zhupengqi/dataset/mscmr/',transform=composed_transforms_ts_grids)
    ...
    # voc_train = sbd.SBDSegmentation(split='train', transform=composed_transforms_tr_grids)
    # voc_val = sbd.SBDSegmentation(split='val', transform=composed_transforms_ts_grids)


    # voc_train = CocoSegmentation(split='train', transform=composed_transforms_tr_grids)
    # voc_val = CocoSegmentation(split='val', transform=composed_transforms_ts_grids)

    # voc_train = CocoSegmentation_City(split='train', transform=composed_transforms_tr_grids)
    # voc_val = CocoSegmentation_City(split='val', transform=composed_transforms_ts_grids)
